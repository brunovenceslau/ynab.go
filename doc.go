...
// [NewWithTokenSource] consults a [TokenSource] before every attempt, so
// a refreshed token is picked up mid-retry. The token travels only in
// the Authorization header and is always redacted from [WithLogger]
// tracing.
//
// Construction never fails: a bad option (an unparseable [WithBaseURL],
// a nil limiter) is stored and every method returns it as
// [*ArgumentError] before any I/O — nothing silently falls back.
//
// # The plan handle
//
...
//
// [PlanIDLastUsed] and [PlanIDDefault] are server-resolved literals
// besides concrete plan ids. The zero Client and the zero Plan are
// unusable by design: handles come only from constructors, so a bound
// plan id can never drift from its services. *Client, *Plan, and all
// services are safe for concurrent use; configuration is set once at
// construction.
//
...
// [ServerKnowledge] cursor. Hand it back with [Since] (or
// [TransactionFilter].Since) to receive only what changed — deletions
// arriving as tombstones that [MergeByID] folds into a local store.
// [SyncState] is the JSON-persistable bundle of cursors, and
...
// pre-flight failures — spec-stated invariants only, like a zero
// [Month], declared length bounds, or split legs that do not sum — are
// [*ArgumentError], and the request is never sent. [Error.RetryAfter]
// of zero means unknown, never "retry immediately".
//
// The built-in pipeline already retries 429 (any verb) and 500/503
// (GET/DELETE unless [RetryPolicy].RetryWrites); [IsRetryable] exposes
// the same classification for custom loops. errors.Is(err,
// [context.Canceled]) and [context.DeadlineExceeded] work through the
// chain, as does [errors.As] for [net.Error] — but the concrete
// transport error types in the chain are not part of the compatibility
// promise.
//
...
// pointed at an [net/http/httptest.Server], and [WithHTTPClient]
// carrying a fake [net/http.RoundTripper] — see their examples. Return
// arities are fixed per operation (they mirror the wire, cursor
// included) and are not expected to change within v1.
package ynab
